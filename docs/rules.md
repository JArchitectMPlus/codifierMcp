...
## 9. CLI Installer (`codifier init`)

**9.1 MCP Config Must Use mcp-remote Proxy Format.** The `.mcp.json` generated by `codifier init` must use the `mcp-remote` npx proxy pattern (`command: "npx"`, `args: ["-y", "mcp-remote", ...]`) with the bearer token passed via `--header`. The `type: "http"` format with a `headers` object must never be used — it requires OAuth2 fields (`authorization_endpoint`) that the Codifier server does not provide, and will fail validation in Claude Code and other MCP clients.

**9.2 Generated Config Must Be Gitignored.** The MCP config file generated by `codifier init` (`.mcp.json` or client-specific path) contains the API key. The init process must ensure this file path is listed in the target project's `.gitignore`. If `.gitignore` does not exist, create it. If it exists but does not list the config path, append it.

**9.3 Config Format Must Be Tested Against Target Clients.** Any change to `buildMcpConfig()` or the generated `.mcp.json` structure must be manually verified against at least Claude Code and one other supported MCP client before merging. Config format errors are silent at install time and only surface when the user tries to connect.